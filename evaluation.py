...
def evaluation_find() :
    ...
    for i in range(0, len(subject_list)):
        # 교과목 입력
        elem = browser.find_element_by_xpath("/html/body/main/div/div/div/div[2]/div[2]/table[1]/tbody/tr[2]/td[1]/input")
        elem.clear()
        elem.send_keys(subject_list[i])

        # 교수님 성함 입력
        elem = browser.find_element_by_xpath("/html/body/main/div/div/div/div[2]/div[2]/table[1]/tbody/tr[2]/td[2]/input")
        elem.clear()
        elem.send_keys(professor_list[i])

        # 인증코드 입력
        code = browser.find_element_by_xpath("/html/body/main/div/div/div/div[2]/div[2]/table[1]/tbody/tr[5]/td/span").text
        ...
        # 해당 교과목 클릭
        browser.find_element_by_xpath("/html/body/main/div/div/div/div[2]/div[2]/table[2]/tbody/tr[1]").click()
        time.sleep(2)
        ...
        for td in tr:
            ...
            row = row[:cut_index]
            if ("평가방법 비율" in row):
                evaluation_rate[subject_list[i]] = row[8:]
    ...
    for key,value in evaluation_rate.items() :
        ...
        evaluation_percent = str(value)[32:]
